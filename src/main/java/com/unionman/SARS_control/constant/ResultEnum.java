package com.unionman.SARS_control.constant;
...
public enum ResultEnum {
    ...
    // 失败
    ERROR(-1, "失败"),
    SYNCHRONIZE_ERROR(-1, "企业微信数据同步失败"),
    SYSTEM_ERROR(-1, "系统错误"),
    ...
    FILE_CAN_NOT_BE_NULL(-1,"文件不能为空"),
    NOT_LOGGED_IN_BROWSER(-2,"未登录"),
    ...
    CANNOT_CONTINUE_TO_AD_SUB_ORGANIZATIONS(5407, "不可继续新增子组织"),

    //5400:产品模块
    PRODUCT_CLASSIFY_EXISTS(5401,"该产品分类已存在"),
    PRODUCT_CLASSIFY_INFO_DOES_NOT_EXIST_OR_HAS_BEEN_DELETED(5402, "该产品分类信息不存在或已被删除"),
    PRODUCT_DETAIL_EXISTS(5403,"该产品已存在"),
    PRODUCT_DETAIL_INFO_DOES_NOT_EXIST_OR_HAS_BEEN_DELETED(5404, "该产品信息不存在或已被删除"),

    //5500:案例模块
    CASE_CLASSIFY_EXISTS(5501,"该案例分类已存在"),
    CASE_CLASSIFY_INFO_DOES_NOT_EXIST_OR_HAS_BEEN_DELETED(5502,"该案例分类信息不存在或已被删除"),
    CASE_DETAIL_EXISTS(5503,"该案例已存在"),
    CASE_DETAIL_INFO_DOES_NOT_EXIST_OR_HAS_BEEN_DELETED(5504, "该案例信息不存在或已被删除"),

    ADD_ACTIVITY_LOG_INFO(5505,"管理员发布活动"),

    // 网站模块
    UNKNOW_WEGSITE(5408,"未知网站，请与管理员联系"),
    ;
